...
# Script 01: Level 1 Synthetic Data Analysis
...
# Level 1 Survey data path
survey_path <- "C:/Users/[username]/Downloads/Synthetic_SAVE_Admin_Proxy_Level1/Synthetic_SAVE_Admin_Proxy_Level1/synthetic_household_and_consumption_data/synthetic_save_household_survey_data/save_household_survey_data_v0-3.csv"

# Level 1 Electricity folder — December 2018
elec_folder <- "C:/Users/[username]/Downloads/Synthetic_SAVE_Admin_Proxy_Level1/Synthetic_SAVE_Admin_Proxy_Level1/synthetic_household_and_consumption_data/"

# ============================================================
# STEP 1 — LOAD SURVEY DATA
# ============================================================
cat("Loading Level 1 survey data...\n")
...
cat("\n--- Survey Mode ---\n")
print(table(survey$surveyMode))

cat("\n--- Missing Values Top 10 ---\n")
missing <- sort(colSums(is.na(survey)),
...
# Final aggregation per household
elec_final <- elec_agg %>%
  group_by(household_id) %>%
  summarise(
    avg_kwh = round(
...
# ============================================================
# STEP 4 — ENERGY SUMMARY
# ============================================================
cat("\n--- Level 1 Energy Summary ---\n")
print(summary(elec_final$avg_kwh))

cat("\nPeak avg:",
    round(mean(elec_final$peak_avg_kwh,
               na.rm = TRUE), 4), "kWh\n")
cat("Off-Peak avg:",
    round(mean(elec_final$offpeak_avg_kwh,
               na.rm = TRUE), 4), "kWh\n")

# ============================================================
# STEP 5 — LINK SURVEY AND ELECTRICITY
# ============================================================
cat("\nLinking datasets...\n")
...
# ============================================================
cat("\n========================================\n")
cat("DESCRIPTIVE ANALYSIS — LEVEL 1\n")
cat("========================================\n")
...
# ============================================================
  cat("\n========================================\n")
  cat("PREDICTIVE ANALYSIS — LEVEL 1\n")
  cat("========================================\n")
...
# ============================================================
# STEP 8 — CREATE AND SAVE GRAPHS
# ============================================================
  cat("\nCreating graphs...\n")

  # Graph 1 — Energy by Trial Group
  g1 <- merged %>%
    group_by(Intervention) %>%
    summarise(avg_kwh = mean(avg_kwh, na.rm = TRUE))

  ggplot(g1,
         aes(x = factor(Intervention),
             y = avg_kwh,
             fill = factor(Intervention))) +
    geom_bar(stat = "identity") +
    labs(title = "Level 1: Energy by Trial Group",
  ...
  ggsave("Graphs/Level1/L1_graph1_trial_group.png",
         width = 8, height = 5)
  cat("Graph 1 saved!\n")

  # Graph 2 — Energy by Deprivation
  g2 <- merged %>%
    group_by(quartile) %>%
    summarise(avg_kwh = mean(avg_kwh, na.rm = TRUE))
  ...
  ggplot(g2,
    ...
    geom_bar(stat = "identity") +
    labs(title = "Level 1: Energy by Deprivation",
         subtitle = "December 2018",
         x = "IMD Quartile",
         y = "Average kWh",
         fill = "Quartile") +
  ...
  ggsave("Graphs/Level1/L1_graph2_deprivation.png",
         width = 8, height = 5)
  cat("Graph 2 saved!\n")
  ...
  ggplot(g3,
         aes(x = Location,
             ...
             fill = Location)) +
    geom_bar(stat = "identity") +
    labs(title = "Level 1: Energy Urban vs Rural",
         subtitle = "December 2018",
         x = "Location",
         y = "Average kWh",
         fill = "Location") +
  ...
  ggsave("Graphs/Level1/L1_graph3_urban_rural.png",
         width = 8, height = 5)
  cat("Graph 3 saved!\n")
  ...
  ggplot(g4,
         aes(x = Period,
             ...
             fill = Period)) +
    geom_bar(stat = "identity") +
    labs(title = "Level 1: Peak vs Off Peak",
         subtitle = "December 2018",
         x = "Period",
         y = "Average kWh",
         fill = "Period") +
  ...
  ggsave("Graphs/Level1/L1_graph4_peak_offpeak.png",
         width = 8, height = 5)
  cat("Graph 4 saved!\n")

  cat("\nAll Level 1 graphs saved!\n")

} else {
  cat("No merged data found!\n")
}

cat("\n========================================\n")
cat("LEVEL 1 ANALYSIS COMPLETE!\n")
cat("========================================\n")
